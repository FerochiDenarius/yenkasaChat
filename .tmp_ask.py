...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Ask Yenkasa Engineering AI questions using Gemini + Chroma RAG."
    )
    parser.add_argument(
        "--persist-dir",
    ...
    parser.add_argument(
        ...
        help="Embedding provider used to embed the query before Chroma search.",
    )
    parser.add_argument(
        "--embedding-model",
    ...
    parser.add_argument(
        ...
        help="Gemini chat model to use for answer generation.",
    )
    parser.add_argument(
        "--temperature",
        type=float,
        default=float(os.getenv("GEMINI_TEMPERATURE", "0.2")),
...
def build_embedding_function(provider: str, model_name: str):
    ...
    try:
        if provider == "huggingface":
            ...
            LOGGER.info(
                "Embedding backend ready provider=%s model=%s init_time=%.2fs",
                provider,
                chosen_model,
        ...
        if provider == "gemini":
            chosen_model = (
                model_name if model_name and model_name != DEFAULT_HF_MODEL else DEFAULT_GEMINI_EMBEDDING_MODEL
            )
            google_api_key = os.getenv("GEMINI_API_KEY") or os.getenv("GOOGLE_API_KEY")
            if not google_api_key:
                raise RuntimeError(
                    "Gemini embeddings requested but GEMINI_API_KEY/GOOGLE_API_KEY is not set."
                )
        ...
        provider_hint = (
            ...
            if provider == "huggingface"
            else "Ensure GEMINI_API_KEY/GOOGLE_API_KEY is set and langchain-google-genai is installed."
        )
        raise RuntimeError(
...
def build_chat_model(model_name: str, temperature: float):
    ...
    try:
        ...
        raise RuntimeError(
            ...
            "venv/bin/python -m pip install -U langchain-google-genai"
        ) from exc
    ...
    LOGGER.info(
        "Gemini chat model ready model=%s init_time=%.2fs",
        model_name,
        time.perf_counter() - started_at,
    )
    return llm
...
def answer_query(
    ...
    if usage_metadata:
        LOGGER.info("Gemini token usage %s", usage_metadata)

    LOGGER.info(
        "Answered query retrieval_elapsed=%.2fs generation_elapsed=%.2fs total_elapsed=%.2fs",
        retrieval_elapsed,
        generation_elapsed,
        total_elapsed,
    )
...
def print_banner(persist_dir: Path, collection_name: str, retrieval_k: int, chat_model: str) -> None:
    ...
    print(f"Gemini Model: {chat_model}")
...
def run() -> int:
    ...
    def handle_query(query: str) -> None:
        try:
            ...
            print("\nError: failed to generate an answer for that query.")
            return
