...
def normalize_content_blocks(value: Any) -> str:
    if value is None:
        return ""
    ...
    if isinstance(value, list):
        parts: list[str] = []
        for item in value:
            normalized = normalize_content_blocks(item)
            if normalized:
    ...
    if isinstance(value, dict):
        for key in ("text", "content", "output_text", "answer"):
            ...
            if candidate:
                normalized = normalize_content_blocks(candidate)
                if normalized:
                    return normalized
        try:
...
def extract_answer_text(response: Any) -> str:
    ...
    if isinstance(response, dict):
        return normalize_content_blocks(response)
    ...
    for attr_name in ("text", "output_text", "answer"):
        ...
        if callable(attr_value):
            try:
                ...
                attr_value = None
        normalized_attr = normalize_content_blocks(attr_value)
        if normalized_attr:
            return normalized_attr

    additional_kwargs = getattr(response, "additional_kwargs", None)
    normalized_kwargs = normalize_content_blocks(additional_kwargs)
    if normalized_kwargs:
        return normalized_kwargs

    try:
        serialized = response.model_dump()  # pydantic-style messages
    except Exception:
        serialized = None

    normalized_serialized = normalize_content_blocks(serialized)
    if normalized_serialized:
        return normalized_serialized
...
def answer_query(
    ...
    if not answer_text:
        answer_text = (
            "No answer text was returned by Vertex AI. The request completed, but the response "
            "content was empty or in an unsupported format."
        )
    ...
    return answer_text.strip(), results, {
