...
def validate_vertex_credentials(project_id: str) -> Path:
    credentials_path_value = os.getenv("GOOGLE_APPLICATION_CREDENTIALS")
    if not credentials_path_value:
        raise RuntimeError(
            "GOOGLE_APPLICATION_CREDENTIALS is not set. Point it to your Vertex AI service account JSON file."
        )

    credentials_path = Path(credentials_path_value).expanduser().resolve()
    if not credentials_path.exists() or not credentials_path.is_file():
        raise RuntimeError(
            f"GOOGLE_APPLICATION_CREDENTIALS does not exist or is not a file: {credentials_path}"
        )
    ...
    try:
        ...
        raise RuntimeError(
            f"Failed to read GOOGLE_APPLICATION_CREDENTIALS JSON file: {credentials_path}"
        ) from exc

    credential_project = credentials_payload.get("project_id")
    credential_type = credentials_payload.get("type", "unknown")
    ...
    try:
        credentials, detected_project = load_credentials_from_file(str(credentials_path))
    except Exception as exc:
        raise RuntimeError(
            f"Failed to load Google credentials from {credentials_path}"
        ) from exc

    resolved_project = detected_project or credential_project or project_id
    if project_id and resolved_project and project_id != resolved_project:
        LOGGER.warning(
            "Vertex project mismatch requested_project=%s credential_project=%s credential_type=%s",
            project_id,
            resolved_project,
            credential_type,
        )
        ...
        LOGGER.info(
            "Vertex credentials validated credential_path=%s credential_type=%s project=%s",
            credentials_path,
            credential_type,
            resolved_project,
        )
    ...
    if credentials is None:
        raise RuntimeError("Google credentials loaded as None, which is invalid for Vertex AI.")

    return credentials_path


def build_chat_model(model_name: str, temperature: float, project_id: str, location: str):
    credentials_path = validate_vertex_credentials(project_id)
    ...
    LOGGER.info(
        "Vertex AI Gemini chat model ready provider=vertex_ai model=%s project=%s location=%s credentials=%s init_time=%.2fs",
        model_name,
        project_id,
        location,
        credentials_path,
