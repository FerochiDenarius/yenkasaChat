...
router.post("/toggle-follow/:targetUserId", verifyToken, async (req, res) => {
  try {
    ...
    if (isFollowing) {
      ...
      // 🎁 Optional: reward coins for following
      try {
        if (rewardCoins) await rewardCoins(userId, "follow", 10, targetUserId);
      } catch (err) {
        console.warn("⚠️ rewardCoins error:", err.message);
      }
    }

    // 🧮 Refresh updated users to recalc counts
    ...
    updatedTarget.followersCount = updatedTarget.followers.length;
    await Promise.all([updatedUser.save(), updatedTarget.save()]);
    ...
    res
      .status(500)
      .json({ message: "Failed to follow/unfollow", error: err.message });
  }
});
