...
gcloud run deploy "$SERVICE_NAME" \
  ...
  --min-instances 0 \
  --max-instances 2 \
  --set-env-vars NODE_ENV=production,MEDIA_STORAGE_PROVIDER=gcs,GCS_MEDIA_BUCKET=yenkasa-media,GOOGLE_CLOUD_PROJECT="$PROJECT_ID",GCS_MAKE_PUBLIC=false,YENKASA_SOCKET_REDIS_ENABLED=true,YENKASA_PRESENCE_REDIS_ENABLED=true,YENKASA_ENABLE_INLINE_MODERATION_WORKERS=false,YENKASA_ENABLE_INLINE_YME_WORKERS=false
