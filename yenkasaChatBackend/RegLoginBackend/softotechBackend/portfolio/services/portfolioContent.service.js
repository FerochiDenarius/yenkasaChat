...
function defaultContent() {
  return {
    ...
    teamMembers: [
      ...
      { id: 'joana-amoquandoh', name: 'Joana Amoquandoh', role: 'Project Manager', department: 'Project Management', status: 'Active Team Member', active: true, photo: '/images/default.png', bio: 'Project manager supporting Yenkasa Soft-O-Tech client delivery and coordination.', background: 'Project manager supporting Yenkasa Soft-O-Tech client delivery and coordination.', skills: ['Project coordination', 'Client communication', 'Requirements tracking', 'Delivery support'], linkedIn: '', email: '', fieldOfStudy: 'Computer Science Student, Accra Technical University (ATU).', major: 'Project coordination, client communication, requirements tracking, and delivery support.' },
    ],
    milestones: [],
  };
}
...
function cleanContent(content) {
  ...
  const cleanedTeam = (teamSource.length ? teamSource : fallback.teamMembers).map((item) => ({
    id: String(item?.id || item?.name || '').trim().toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-+|-+$/g, ''),
    name: String(item?.name || '').trim(),
    role: String(item?.role || '').trim(),
    department: String(item?.department || '').trim(),
    status: String(item?.status || 'Active Team Member').trim(),
    active: item?.active !== false,
    photo: String(item?.photo || item?.image || '/images/default.png').trim(),
    bio: String(item?.bio || item?.background || '').trim(),
    background: String(item?.background || '').trim(),
    skills: normalizeArray(item?.skills).map((skill) => String(skill).trim()).filter(Boolean),
  ...
  const teamById = new Map(fallback.teamMembers.map((member) => [member.id, member]));
  cleanedTeam.forEach((member) => {
    const normalizedId = member.id === 'joana-amoquandoh-ayeyi' ? 'joana-amoquandoh' : member.id;
    teamById.set(normalizedId, { ...(teamById.get(normalizedId) || {}), ...member, id: normalizedId });
  });
  const teamMembers = Array.from(teamById.values());
  return {
...
async function updateTeamMemberPhoto(teamMemberId, photoUrl, actor = {}) {
  ...
  const nextContent = {
    ...content,
    teamMembers: exists ? teamMembers : [...teamMembers, { id, name: id.replace(/-/g, ' '), role: 'Team Member', department: '', status: 'Active Team Member', active: true, photo: url, bio: '', background: '', skills: [], linkedIn: '', email: '', fieldOfStudy: '', major: '' }],
  };
