...
router.get('/engineering/dashboard', portalAuth, async (req, res) => {
  try {
    const dashboard = await portal.engineeringDashboard(req.portalUser, {
...
router.get('/admin/github/connection', portalAuth, portalAdminOnly, async (req, res) => {
  try {
    const connection = await portal.githubConnection(req.portalUser);
...
router.get('/admin/github/connect-url', portalAuth, portalAdminOnly, async (req, res) => {
  try {
    const result = await portal.createGitHubOAuthUrl(req.portalUser, {
      projectId: req.query.projectId,
...
router.get('/admin/github/oauth/callback', async (req, res) => {
  try {
    await portal.handleGitHubOAuthCallback({
      code: req.query.code,
      state: req.query.state,
      origin: requestOrigin(req),
    });
    res.redirect(302, '/admin?panel=github-activity&github=connected');
  } catch (error) {
    res.status(error.statusCode || 500).send(`GitHub connection failed: ${error.message || 'Request failed.'}`);
  }
});

router.get('/admin/github/repositories', portalAuth, portalAdminOnly, async (req, res) => {
  try {
    const repositories = await portal.listGitHubRepositories(req.portalUser);
...
router.get('/admin/github/branches', portalAuth, portalAdminOnly, async (req, res) => {
  try {
    const branches = await portal.listGitHubBranches(req.portalUser, req.query.repository);
...
router.get('/admin/github/folders', portalAuth, portalAdminOnly, async (req, res) => {
  try {
    const folders = await portal.listGitHubFolders(req.portalUser, {
...
router.get('/admin/projects/:projectId/repositories', portalAuth, portalAdminOnly, async (req, res) => {
  try {
    const repositories = await portal.listProjectRepositoryMappings(req.params.projectId);
    const activity = await portal.listProjectGitHubActivity(req.params.projectId);
    res.json({ success: true, repositories, activity });
  } catch (error) {
    sendError(res, error);
  }
});

router.post('/admin/projects/:projectId/repositories', portalAuth, portalAdminOnly, async (req, res) => {
  try {
    const repository = await portal.saveProjectRepositoryMapping(req.params.projectId, req.body || {}, {
      ...req.portalUser,
      email: req.portalUser.email,
      role: req.portalUser.role || req.portalUser.userType,
    });
...
router.post('/admin/repositories/:mappingId/sync', portalAuth, portalAdminOnly, async (req, res) => {
  try {
    const activity = await portal.syncProjectRepositoryMapping(req.params.mappingId, {
      ...req.portalUser,
      email: req.portalUser.email,
      role: req.portalUser.role || req.portalUser.userType,
    });
...
router.delete('/admin/repositories/:mappingId', portalAuth, portalAdminOnly, async (req, res) => {
  try {
    const result = await portal.removeProjectRepositoryMapping(req.params.mappingId, {
      email: req.portalUser.email,
      role: req.portalUser.role || req.portalUser.userType,
    });
    res.json({ success: true, ...result });
...
router.post('/admin/payments', portalAuth, portalAdminOnly, upload.single('receipt'), async (req, res) => {
  try {
    const receiptFile = req.file
      ? await portal.uploadPaymentReceipt(req.file, req.body || {}, {
        email: req.portalUser.email,
        role: req.portalUser.role || req.portalUser.userType,
      })
      : null;
    const payment = await portal.recordPayment({
      ...(req.body || {}),
      ...(receiptFile ? { receiptUrl: receiptFile.url, receiptFile } : {}),
    }, {
      email: req.portalUser.email,
      role: req.portalUser.role || req.portalUser.userType,
    });
